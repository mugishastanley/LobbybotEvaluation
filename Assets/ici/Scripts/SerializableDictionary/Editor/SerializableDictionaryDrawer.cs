...
public abstract class SerializableDictionaryDrawer<K, V> : PropertyDrawer {
	...
	public override void OnGUI (Rect position, SerializedProperty property, GUIContent label) {
		...
		// Using BegingProperty / EndProperty on the parent property means that
		// prefab override logic works on the entire property.
		EditorGUI.BeginProperty(position, label, property);

		// Calculate rects and draw fields
		...
		if (property.isExpanded) {
			...
			for (int i = 0; i < numLines; i++) {
				...
				if (GUI.Button(secondLine_button, "Remove")) {
					...
					for (int j = i+1; j < keysProp.arraySize; j++) {
